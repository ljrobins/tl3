...
def make_weekly_data(search_dir: str) -> dict:
    files = [os.path.join(search_dir, x) for x in os.listdir(search_dir) if 'FIN' in x and x.endswith('.SP3')]
    if not len(files):
        files = [os.path.join(search_dir, x) for x in os.listdir(search_dir) if 'RAP' in x and x.endswith('.SP3')]
    ...
    for file in files:
        with open(file, 'r') as f:
            for line in f:
                if line.startswith('*  '):
                    cur_time = datetime.datetime.strptime(line, "*  %Y  %m %d %H %M %S.%f00 ")
                    tx = mr.itrf_to_j2000(cur_time)
                if line.startswith("P"):
                    sat_identifier = line[1:4]
                    pos_itrf = tuple([float(x) for x in ' '.join(line[5:].split()).split()][:-1])
                    pos_j2000 = (tx @ np.array(pos_itrf)).tolist()
                    if not sat_identifier in week_data:
...
if __name__ == "__main__":
    ...
    weeks_sorted = sorted([x for x in os.listdir('data_gnss') if x.isdigit()], 
