...
t1 = time.time()
res = cur.execute(
    f"SELECT {', '.join(sel_cols)} FROM TLE WHERE NORAD_CAT_ID={norad_id} AND EPOCH > {repr(epoch_start)} AND EPOCH < {repr(epoch_end)} ORDER BY EPOCH"
)
x = res.fetchall()
df = pl.DataFrame(x)
df = df.rename({k: v for k, v in zip(df.columns, sel_cols)})
df = df.with_columns([pl.col('EPOCH').str.to_datetime('%Y-%m-%d %H:%M:%S%.f')])
print(time.time() - t1)


def init_sat_from_row(row):
    days_since = (
        row['EPOCH'][0] - datetime.datetime(1949, 12, 31, 1, 1, 00, 00)
    ).total_seconds() / 86400
    sat = Satrec()
