...
url = lambda date: f'https://nasa-public-data.s3.amazonaws.com/iss-coords/{date.strftime("%Y-%m-%d")}/ISS_OEM/ISS.OEM_J2K_EPH.txt'
save_dir = 'data_iss'
...
for days_back in range((now - end_date).days):
    ...
    try:
        tm.save_file_from_url(durl, save_dir, save_name=save_name)
