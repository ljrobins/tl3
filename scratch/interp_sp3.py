...
if len(dfs) > 1:
    ...
    df = dfs[0]
df = df.filter(
    (pl.col('date') >= d1_with_buffer) & (pl.col('date') <= d2_with_buffer)
).sort('date')
arr = df.select(pl.exclude('date').fill_null(np.inf)).to_numpy()
epsec = (
    df.select(
        pl.col('date').diff().dt.total_seconds().cum_sum().fill_null(0).cast(pl.Float64)
    )
    .to_numpy()
    .flatten()
)
...
t1 = time.time()
interpers.append(
    ('linear', interp1d(epsec_fit, arr_fit, copy=False, axis=0, kind='linear'))
)
interpers.append(
    ('cubic spline', interp1d(epsec_fit, arr_fit, copy=False, axis=0, kind='cubic'))
)
interpers.append(
    (
        'quadratic spline',
        interp1d(epsec_fit, arr_fit, copy=False, axis=0, kind='quadratic'),
    )
)
interpers.append(('akima', Akima1DInterpolator(epsec_fit, arr_fit)))
interpers.append(
    ('modified akima', Akima1DInterpolator(epsec_fit, arr_fit, method='makima'))
)
